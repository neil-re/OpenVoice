...
def _force_utf8_console():
    # 1) Evita que la consola falle al imprimir no-ASCII
    try:
        sys.stdout.reconfigure(encoding="utf-8", errors="replace")
        sys.stderr.reconfigure(encoding="utf-8", errors="replace")
    except Exception:
        # Python <3.7 o entornos sin reconfigure
        sys.stdout = codecs.getwriter("utf-8")(sys.stdout.buffer, errors="replace")
        sys.stderr = codecs.getwriter("utf-8")(sys.stderr.buffer, errors="replace")

    # 2) (Opcional) Forzar codepage UTF-8 en Windows
    if os.name == "nt":
        try:
            import ctypes
...
def main():
    ...
    ap.add_argument("--audio", "--input", dest="audio", required=True,
                    help="Ruta del archivo de audio (m4a/mp3/wav). Alias: --input")
    ...
    # Normalizar/validar compute_type
    valid_compute = {"int8", "int8_float16", "int8_float32", "float16", "float32"}
    if args.compute not in valid_compute:
        print(f"[WARN] compute_type '{args.compute}' no válido. Usando 'int8'.")
        args.compute = "int8"
    ...
    # --- construir kwargs sin None para evitar TypeError en CTranslate2 ---
    model_kwargs = dict(
        model_size_or_path=args.model,  # <--- clave correcta
        device="cpu",
        compute_type=args.compute,
    )

    if args.cpu_threads is not None:
        model_kwargs["cpu_threads"] = int(args.cpu_threads)  # evitar None

    model_kwargs["num_workers"] = int(args.num_workers)

    print(f"[i] Cargando modelo: {args.model} | device=cpu | compute_type={args.compute} | "
          f"cpu_threads={args.cpu_threads} | num_workers={args.num_workers}")
    ...
    if args.emit_srt:
        print(f"  - Subtitulos SRT: {out_srt}")
    if args.emit_json:
